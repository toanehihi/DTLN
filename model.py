...
class audio_generator():
    ...
    def __init__(self, path_to_input, path_to_s1, len_of_samples, fs, train_flag=False, file_mapping_csv=None):
        '''
        ...
        if file_mapping_csv and os.path.exists(file_mapping_csv):
    ...
    def load_file_mapping(self):
        ...
        CSV format: fileid,noisy,clean
        '''
        self.file_mapping = {}
        with open(self.file_mapping_csv, 'r') as f:
            reader = csv.DictReader(f)
            for row in reader:
                self.file_mapping[row['noisy']] = row['clean']
        print(f"Loaded {len(self.file_mapping)} file mappings from {self.file_mapping_csv}")
    ...
    def create_generator(self):
        ...
        for file in self.file_names:
            ...
            # determine clean file name (use mapping if available)
            if self.file_mapping and file in self.file_mapping:
                clean_file = self.file_mapping[file]
            else:
                clean_file = file  # assume same name if no mapping
...
class DTLN_model():
    ...
    def train_model(self, runName, path_to_train_mix, path_to_train_speech, \
                    path_to_val_mix, path_to_val_speech, train_file_mapping=None, val_file_mapping=None):
        '''
        Method to train the DTLN model.
        
        Args:
            runName: name of the training run
            path_to_train_mix: path to training noisy files
            path_to_train_speech: path to training clean files
            path_to_val_mix: path to validation noisy files
            path_to_val_speech: path to validation clean files
        ...
        generator_input = audio_generator(path_to_train_mix, 
                                          ...
                                          file_mapping_csv=train_file_mapping)
        ...
        generator_val = audio_generator(path_to_val_mix,
                                        ...
                                        file_mapping_csv=val_file_mapping)
